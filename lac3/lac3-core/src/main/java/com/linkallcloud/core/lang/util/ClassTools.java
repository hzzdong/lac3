...
import com.linkallcloud.core.Lac2Info;
...
public class ClassTools {
    ...
    private static ClassLoader rcClassLoader;
    static {
        rcClassLoader = Lac2Info.class.getClassLoader();
        //当使用JavaSE是,如果RC通过bootClassLoader加载,那么就会为null
        if (rcClassLoader == null)
            try {
                rcClassLoader = ClassLoader.getSystemClassLoader();
            }catch (Throwable e) {}
    }
    
    /**
     * 获取rc.jar的ClassLoader的方法
     */
    public static ClassLoader getClassLoader() {
        return rcClassLoader;
    }
    
    @Deprecated
    public static void setNutClassLoader(ClassLoader nutClassLoader) {
        ClassTools.rcClassLoader = nutClassLoader;
    }
}
