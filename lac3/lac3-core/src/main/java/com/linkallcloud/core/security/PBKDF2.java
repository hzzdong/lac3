...
/**
 * This class implements functionality to generate and validate PBKDF2 hashes for mosquitto-auth-plug.
 *
...
public class PBKDF2 {
    ...
    /**
     * This method compares a plain password and a PBKDF2 password (in mosquitto-auth-plug format) to know whether the
     * password match the PBKDF2 hash.
     *
     * @author Manuel Domínguez Dorado - manuel.dominguez@enzinatec.com
     * @param plainPassword
     *            Tha plain password to be compared to the PBKDF2 hash.
     * @param hashedPasword
     *            The PBKDF2 password in mosquitto-auth-plug format (usualli it is stored in a MySQL database).
    ...
    public boolean isValidPassword(String plainPassword, String hashedPasword) {
        ...
        KeySpec ks = new PBEKeySpec(plainPassword.toCharArray(), encodedSalt, encodedIterations, PBKDF2.KEY_LENGTH);
        SecretKey s = null;
        try {
    ...
    /**
     * This method creates a new PBKDF2 password (in mosquitto-auth-plug format) from a plain password.
     *
     * @author Manuel Domínguez Dorado - manuel.dominguez@enzinatec.com
     * @param plainPassword
     *            The plain password used to generate the corresponding PBKDF2 password (in mosquitto-auth-plug) format.
     * @return The generated PBKDF2 password in mosquitto-auth-plug format (usually, it will be stored in a MySQL
     *         database).
     * @since 1.0
     */
    public String createPassword(String plainPassword) {
        byte someBytes[] = new byte[PBKDF2.SALT_LENGTH];
        ...
        KeySpec ks = new PBEKeySpec(plainPassword.toCharArray(), encodedSalt.getBytes(), PBKDF2.ITERATIONS,
                PBKDF2.KEY_LENGTH);
        SecretKey s;
        try {
            s = f.generateSecret(ks);
            String encodedKey = Base64.getEncoder().encodeToString(s.getEncoded());
            String hashedKey = "PBKDF2$sha256$" + PBKDF2.ITERATIONS + "$" + encodedSalt + "$" + encodedKey;
            return hashedKey;
