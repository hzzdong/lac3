package com.linkallcloud.core.conf;
...
import com.linkallcloud.core.el.opt.custom.CustomMake;
 ...
 * <li>与JSON 相同, 配置项的值你可以转换成任意你想要的类型. 包括泛型, 可以使用 {@link RcType}
 * </ul>
 * 
 */
public class RcConf {
    ...
    private static final String DEFAULT_CONFIG = "com/linkallcloud/core/core/conf/RcDefaultConfig.js";
    ...
    private volatile static RcConf conf;

    private static RcConf me() {
        if (null == conf) {
            synchronized (RcConf.class) {
                if (null == conf)
                    conf = new RcConf();
            }
        }
