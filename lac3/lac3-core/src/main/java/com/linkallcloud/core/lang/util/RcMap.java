...
public class RcMap extends LinkedHashMap<String, Object> implements RcBean {

    public static RcMap WRAP(Map<String, Object> map) {
        if (null == map)
            return null;
        if (map instanceof RcMap)
            return (RcMap) map;
        return new RcMap(map);
    }
    ...
    public RcMap(Map<String, Object> map) {
        super();
        this.putAll(map);
    }

    public RcMap(String json) {
        super();
        this.putAll(Lang.map(json));
    }

    public RcMap(String key, Object value) {
        super();
        put(key, value);
    }
    ...
    public static RcMap NEW() {
        return new RcMap();
    }

    public static RcMap WRAP(String json) {
        return new RcMap(json);
    }
     ...
     * @return 新 Map
     */
    public RcMap pickAndRemove(String... keys) {
        if (keys.length == 0)
            return new RcMap();
        RcMap re = new RcMap();
        for (String key : keys) {
     ...
     * @return 新 Map
     */
    public RcMap pickBy(String regex) {
     ...
     * @return 新 Map
     */
    public RcMap pickBy(Pattern p, boolean isNot) {
        // 一定不匹配
        if (null == p) {
            return isNot ? this.duplicate() : new RcMap();
        }
     ...
     */
    public RcMap pickAndRemoveBy(String regex) {
        if (Strings.isBlank(regex))
            return new RcMap();
        boolean isNot = regex.startsWith("!");
        Pattern p = Pattern.compile(isNot ? regex.substring(1) : regex);
     ...
     * @return 新 Map
     */
    public RcMap pickAndRemoveBy(Pattern p, boolean isNot) {
        // 一定不匹配
        if (null == p) {
            if (isNot) {
                RcMap re = this.duplicate();
                this.clear();
                return re;
            } else {
                return new RcMap();
     ...
     * @return 自身以便链式赋值
     */
    public RcMap putDefault(String key, Object dft) {
        if (!this.has(key)) {
            this.put(key, dft);
        }
        return this;
     ...
     */
    @SuppressWarnings("unchecked")
    public RcMap addv(String key, Object value) {
        Object obj = get(key);
        if (null == obj) {
            put(key, value);
     ...
     * @since 1.b.51
     */
    public RcMap putv(String key, Object value) {
        return addv(key, value);
    }

    public RcMap setv(String key, Object value) {
        this.put(key, value);
        return this;
    }
    ...
    public RcBean setAll(Map<String, Object> map) {
        this.putAll(map);
        return this;
    }

    public RcMap setMap(Map<?, ?> map, boolean ignoreNullValue) {
     ...
     */
    public RcMap mergeWith(Map<String, Object> map) {
     ...
     */
    @SuppressWarnings("unchecked")
    public RcMap mergeWith(Map<String, Object> map, boolean onlyAbsent) {
        for (Map.Entry<String, Object> en : map.entrySet()) {
            ...
            if (null != myVal && myVal instanceof Map && val instanceof Map) {
                ...
                RcMap m2 = RcMap.WRAP(m0).mergeWith(m1, onlyAbsent);
                // 搞出了新 Map，设置一下
                if (m2 != m0)
                    this.put(key, m2);
            }
     ...
     * @return 当前的NutMap实例
     */
    public RcMap setnx(String key, Object val) {
        if (!containsKey(key))
            setv(key, val);
        return this;
    }
     ...
     */
    public RcMap setnxAll(Map<String, Object> map) {
