...
public abstract class BaseWebLogFace<T extends BusiLog, S extends ILacLogManager<T>> {
	...
	public @ResponseBody Object fetchById(IdFaceRequest faceReq, Trace t, SessionUser su) {
		if (faceReq.getId() == null || Strings.isBlank(faceReq.getUuid())) {
			throw new BizException(Exceptions.CODE_ERROR_PARAMETER, "参数错误");
		}
		T entity = doFetch(t, faceReq.getId(), su);
		return convert(t, "fetch", faceReq, entity);
	}

	protected T doFetch(Trace t, Long id, SessionUser su) {
		if (id == null) {
			throw new BizException(Exceptions.CODE_ERROR_PARAMETER, "参数错误");
		}
		return manager().fetchById(t, id);
	}
